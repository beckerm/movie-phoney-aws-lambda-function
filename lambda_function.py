...
def send_email(the_sender, the_recipient, the_subject, the_body_html):
    ...
    try:
        response = client.send_email(
            ...
            Message={'Body': {'Html': {'Charset': CHARSET, 'Data': BODY_HTML, }, }, 'Subject': {
...
def get_movie_data(x_days, x_zip):
    ...
    movie_url = 'http://data.tmsapi.com/v1.1/movies/showings?startDate=' + \
        show_day + '&numDays=' + x_days + '&zip=' + x_zip + '&api_key=' + api_key
    ...
    for r in movie_json:
        for ii in r['showtimes']:
            ...
            movie_list.append([str(ii['theatre']['name']).strip(), str(
                ii['dateTime']).strip(), str(r['title']).strip() + ' (Dir: ' + director_str + ')'])
...
def lambda_handler(event, context):
    ...
    send_email("Movie Phoney <filmnerd@email.com>",
               send_to, subject_line, movie_msg)
